...
/*   Updated: 2024/01/05 14:16:33 by fabi             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!my_sleep_until(philo->next_eat_t, philo))
		return (false);
	...
	if (!change_status(philo, "is eating"))
	{
		drop_forks(philo);
		return (false);
	}
	...
	my_sleep_no_exit(philo->current_t + philo->eat_dur);
	if (drop_forks(philo))
	{
